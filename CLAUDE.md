...
- **Do not "fix" CA1862 inside EF Core queries** — the `Name.ToLower() == lowerName` uniqueness checks in `Create/UpdateVaultCommand` and `Create/UpdateUserGroupCommand` sit in LINQ expression trees that EF translates to SQL. The analyzer suggests `string.Equals(x, y, StringComparison.OrdinalIgnoreCase)`, which EF Core cannot translate — it throws at runtime on both Npgsql and the SQLite test provider. SonarCloud reports these as `external_roslyn:CA1862` (INFO); they are false positives in query context, same class as the trigram gotcha above.
- **`css:S8776` on `frontend/src/styles.css` is suppressed in the SonarCloud scan** — Sonar's CSS parser does not model Tailwind v4's `@utility` at-rule as a scoping root, so `&:hover` / `&:active` inside `@utility btn-interactive` read as unscoped. The nesting is the documented Tailwind v4 way to give a custom utility its variants. The exclusion lives in `.circleci/config.yml`'s `sonarscanner begin` step.
