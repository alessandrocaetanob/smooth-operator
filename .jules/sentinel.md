...
## 2026-05-18 - Missing Authorization on Metrics Endpoint
**Vulnerability:** The Prometheus `/metrics` endpoint was relying entirely on a `UseWhen` condition and returning `200 OK` without validating an authorization token in environments where `Metrics:BearerToken` was unconfigured, leaving it publicly accessible.
**Learning:** Endpoints that expose internal metrics can leak sensitive operational information if left public. Custom bearer token validation middleware that bypasses the default pipeline authorization risks failing open or being bypassed if the configuration logic falls back to ignoring the token entirely when it is not provided.
**Prevention:** Apply a fail-closed `.RequireAuthorization()` policy on the metrics endpoint using a custom assertion policy that returns `false` (unauthorized) when the configured token is null or whitespace, enforcing authentication in all environments.
