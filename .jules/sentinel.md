...
## 2026-05-10 - Unauthenticated Metrics Endpoint
**Vulnerability:** The Prometheus `/metrics` endpoint mapped via `app.MapMetrics("/metrics")` was publicly accessible without integrating with ASP.NET Core's authorization pipeline, despite a custom bearer token middleware `HasValidMetricsBearerToken`.
**Learning:** Even if custom middleware checks for a bearer token, mapping an endpoint without `.RequireAuthorization()` bypasses standard authorization features and policies. When adding `.RequireAuthorization()` to an endpoint guarded by custom auth logic, the custom logic must run *before* `app.UseAuthorization()` to correctly populate `ctx.User` and avoid 401 Unauthorized errors from the framework.
**Prevention:** Ensure that all sensitive endpoints explicitly call `.RequireAuthorization()`. If a custom authentication mechanism (like a static metrics token) is used instead of the default scheme, ensure the custom middleware sets `ctx.User` to an authenticated `ClaimsPrincipal` *before* the ASP.NET Core `AuthorizationMiddleware` executes in the pipeline.
